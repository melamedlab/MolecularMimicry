...
function findKmer(this::KmerLocator, kmers::Array{String},
                  nMismatch::Int64=0; indels::Bool=false)::DataFrame
    return vcat([findKmerInner(this, Array{UInt8}(kmer),
                               nMismatch, indels=indels) for kmer in kmers]...)
end
...
kmers = Dict{String,Vector{Tuple{String,Integer}}}();
for (seqName, seq) in queryFasta
    for i = 1:(length(seq)-k+1)
        ikm = seq[i:(i+k-1)]
        if !(ikm in keys(kmers))
            kmers[ikm] = [(seqName, i)]
        else
            push!(kmers[ikm], (seqName, i))
        end
    end
end
...
locs = findKmer(kmLocator, collect(keys(kmers)), nMismatch);
println("query\tquery_seqname\tquery_start\ttarget_seqname\ttarget_start\ttarget")
for i = 1:size(locs, 1)
    for (querySeqName, queryPos) in kmers[locs[i, 1]]
        println(locs[i, 1] * "\t" * querySeqName * "\t" * string(queryPos) *
                "\t" * locs[i, 2] * "\t" * string(locs[i, 3]) * "\t" * locs[i, 4])
    end
end

# julia fasta_kmer_locations_query_tracking.jl UP000005640_9606.fasta.gz UP000000354_694009.fasta.gz 10 1

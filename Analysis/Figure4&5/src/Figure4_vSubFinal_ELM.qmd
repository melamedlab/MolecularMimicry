...
## Figure 4 - Biological Enrichment
...
for(active_file in files_to_read){
  ...
  if(mode == "tight_elm"){
    active_data <- active_data %>%
      ...
      group_by(query, query_seqname, query_start, mismatches, entrezgene_id) %>%
      slice_max(in_elm) %>%
      distinct(query, query_seqname, query_start, mismatches, entrezgene_id, in_elm) %>%
      filter(!in_elm)
  } 
...
saveRDS(out_final, "../Figure4/output/KEGG_results_2024-08-04_elm.rds")
...
write.csv(df_out3, "..//Figure4/output/KEGG_results_2024-08-04_elm.csv")
...
gg1 <- plot.data %>%
  ...
  #filter(virus %in% metadata$Proteome.ID[metadata$Family_Viral_Zone %in% c("Herpesviridae", "Poxviridae")]) %>%
  ...
  ggplot(aes(x=Description, y=Taxon.mnemonic, size = log10.p.adjust, fill = log10.p.adjust)) +
...
gg1 %>% insert_left(gg_tree, width =  c(0.07))
ggsave(paste0(out_dir, "Figure4A.pdf"), width = 13, height = 9) ## was 18 (or 16?) by 8
```
...
ggsave(paste0(out_dir, "Figure4B.pdf"), width = 5, height = 5)
```
...
xlsx::write.xlsx(supp_data, paste0(out_dir, "TableS5.xlsx"), sheetName = "Perm2_Chr_Data",  append = FALSE)
...
ggsave(paste0(out_dir, "Figure4C.pdf"), width =5.5, height = 5)
```
...
xlsx::write.xlsx(supp_data, paste0(out_dir, "TableS5.xlsx"),

...
## The following directory contains all fastas of pathogen proteomes of interest
...
files_ran <- paste0(gsub(".*\\/", "", list.dirs("/stor/work/Ehrlich_COVID19/Cole/Molecular_Mimicry/epitope_alignment/Viral_Proteomes/Permutation_Experiment_AAClass")), ".fasta.gz")
viral.files <- viral.files[!viral.files %in% files_ran]

hydrophobic <- rep("1", 5); names(hydrophobic) <- c("A", "V", "L", "I", "M")
hydrophilic <- rep("2", 4); names(hydrophilic) <- c("N", "Q", "S", "T")
positive <- rep("3", 3); names(positive) <- c("K", "R", "H")
negative <- rep("4", 2); names(negative) <- c("D", "E")
aromatic <- rep("5", 3); names(aromatic) <- c("F", "Y", "W")
## Special cases includ G, P, and C which are not being shuffled
## Essentially this forms the dictionary, note I'm using numbers because other letters could complicate the pattern sub latter
